...
from models import base_mobile as base
from utils import *
...
class Policy32(nn.Module):
    ...
    def forward(self, x):
        x = self.features.forward_full(x) #0.9
        probs = torch.sigmoid(self.logit(x)).view(-1, self.num_blocks, self.num_of_actions) #0.3
