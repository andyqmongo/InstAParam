...
parser.add_argument('--model', default='InstAParam', choices=['InstAParam', 'InstAParam-single'])
parser.add_argument('--dset_name', default=None, required=True, choices=['C10', 'C100', 'Tiny', 'Fuzzy-C10'])
parser.add_argument('--data_dir', default='./data', help='data directory')
parser.add_argument('--load', default=None, help='checkpoint to load agent and net from')
...
def get_reward(preds, targets, policy, elasped):
    ...
    sparse_reward = torch.log( 8*5 - elasped.cuda().data )
...
def train_online_and_test(trainLoaders, testLoaders):
    ...
    for task in range(num_tasks):
        ...
        if args.dset_name == 'C10':
            ...
            policies_real_mean.append(torch.sum(torch.sum(torch.stack(policies), dim=0, keepdim=True)[0], dim=0, keepdim=True)[0])
            continue
            ...
            print('Pretrain done')
            policy_zero = Variable(torch.zeros(args.batch_size, instanet.num_of_blocks, instanet.num_of_actions)).cpu()
            policies_real_mean.append(torch.sum(torch.sum(torch.stack(policy_zero), dim=0, keepdim=True)[0], dim=0, keepdim=True)[0])
            continue

        if task > 1 or (task > 0 and args.dset_name=='C10'):
            #Calculate history matrix H to encourage exploration 
            history_policies = Variable(torch.sum(torch.stack(policies_real_mean[:task]), dim=0, keepdim=True)[0]).cuda(non_blocking=True)
        ...
        for _, (inputs, targets) in tqdm.tqdm(enumerate(trainloaders[task]), total=len(trainloaders[task])):
            iteration += 1
            for it in range(args.iter_per_batch):
                instanet.assign_mask()
            ...
            torch.save(state, args.cv_dir+'/Best_Task_%d.t7' % (task))
        ...
        cur_prm = policies_real_mean[task].cpu().numpy()
        print(cur_prm)

        df = pd.DataFrame(cur_prm)
        df.to_excel('{}/policies_real_mean_task{}.xlsx'.format(args.cv_dir, task))
...
def test(testloaders, repro_oneshot=False, test_task=-1, cur_task=-1):
    ...
    assert cur_task >= 0
    if os.path.exists('{}/Best_Task_{}.t7'.format(args.cv_dir, cur_task)):
        ckpt_net = torch.load('{}/Best_Task_{}.t7'.format(args.cv_dir, cur_task))
        instanet_.load_state_dict(ckpt_net['instanet'], strict=False)
        instanet_.eval().cuda()
    ...
    for task in loop_through_task:
        matches, policies = [], []
        if os.path.exists('{}/Best_Task_{}.t7'.format(args.cv_dir, task)):
            ckpt = torch.load('{}/Best_Task_{}.t7'.format(args.cv_dir, task))
            agent_.load_state_dict(ckpt['agent'])
            agent_.eval().cuda()
